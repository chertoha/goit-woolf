...
def rabin_karp_search(text, pattern):
    ...
    for i in range(n - m + 1):
        # print(f"pattern_hash: {pattern_hash},\t current_hash: {
        #       current_hash},\t pattern: {pattern},\t current text: {text[i:i + m]}")
        ...
        if i < n - m:
            current_hash = (
                current_hash - ord(text[i]) * (base ** (m - 1))) % prime
            current_hash = (current_hash * base + ord(text[i + m])) % prime
            # Забезпечуємо, щоб хеш був додатнім
